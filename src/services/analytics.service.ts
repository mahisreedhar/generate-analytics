...
import {
  extractAnthropometricValues,
  normalizeGender,
  resolveId,
} from '../utils/extract.util.js';
import { computeAgeInMonths } from '../utils/age.util.js';
import { computeNutritionLabels } from '../utils/nutrition.util.js';
...
export async function processVisit(
  ...
  log(
    `[analytics] Visit loaded: phase=${visit.phase}, date=${visit.visit_date}`
  );
  ...
  if (!childId) {
    errorLog(
      `[analytics] visit.child is missing for visit ${visitId}. Aborting.`
    );
    return;
  }
  ...
  log(
    `[analytics] Child loaded: id=${child.$id}, gender=${child.gender}, date_of_birth=${child.date_of_birth}`
  );
  ...
  if (weight === null || height === null) {
    log(
      `[analytics] Incomplete anthropometric data for visit ${visitId} ` +
        `(weight=${weight}, height=${height}). Skipping.`
    );
    return;
  }
  ...
  const ageInMonths = computeAgeInMonths(child.date_of_birth, visit.visit_date);
  if (ageInMonths < 0) {
    errorLog(
      `[analytics] Negative age (${ageInMonths.toFixed(2)} months) for visit ${visitId}. ` +
        `date_of_birth=${child.date_of_birth}, visit_date=${visit.visit_date}. Skipping.`
    );
    return;
  }
  log(`[analytics] Age in months: ${ageInMonths.toFixed(4)}`);

  const resolvedMeasure = measure || (ageInMonths <= 18 ? 'l' : 'h');

  // ── 7. Call WHO Anthro API ────────────────────────────────────────────────
  const anthro = await callAnthroApi(
    config.anthroApiUrl,
    { sex, ageInMonths, weight, height, measure: resolvedMeasure },
    log,
    errorLog
  );
  log(
  ...
  // ── 8. Compute nutrition Labels ───────────────────────────────────────────
  const nutritionLabels = computeNutritionLabels(
    anthro.zWeightForHeight,
    anthro.zWeightForAge,
    anthro.zHeightForAge,
    anthro.zBmiForAge
  );
  log(`[analytics] Nutrition Labels: ${nutritionLabels.join(', ')}`);
  ...
  const payload: Record<string, unknown> = {
    ...
    nutrition_labels: nutritionLabels,
  };
  ...
  if (options.dryRun) {
    log(
      `[analytics] Dry run enabled; skipping analytics upsert for visit ${visitId}`
    );
    log(
      `[analytics] Dry run analytics payload:\n${JSON.stringify(payload, null, 2)}`
    );
    return;
  }
  ...
  log(
    `[analytics] Analytics document ${action} successfully for visit ${visitId}`
  );
}
